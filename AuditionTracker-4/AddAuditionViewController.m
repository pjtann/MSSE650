...
@property(nonatomic, assign, getter=isTranslucent) BOOL translucent; // used with pickers toolbars as property to give it a background shading, dont' think I ended up using this?
...
// used with above toolbar on picker - I DON"T THINK I NEED THIS - DOESN"T SEEM TO DO ANYTHING
...
- (BOOL)shouldPerformSegueWithIdentifier:(NSString *)identifier sender:(id)sender{
    ...
    if (sender != self.saveAuditionButton) {
        return YES;
    }else{
        
        if ((sender == self.saveAuditionButton) && ([identifier isEqualToString:@"segueFromAddSaveToList"])){
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    // test for no value or original display value of Title field
    if (self.auditionTitle.text.length > 0 && (![self.auditionTitle.text isEqualToString: _oldTitle])){
